...
public class LDExamSchedule {
    public static void main(String[] args) throws IOException {
        ...
        long newStucount = Files.lines(Paths.get("car-s-91.stu")).filter(line -> line.length() > 0).count();
        int totalStu = (int)newStucount;
        System.out.println("new Total Student: " + totalStu);
        ...
        long newCrscount = Files.lines(Paths.get("car-s-91.crs")).filter(line -> line.length() > 0).count();
        int totalCrs = (int)newCrscount;

        System.out.println("new Total Course: " + totalCrs);
        ...
        graphColoring gclr = new graphColoring();
        int minSlot = gclr.setColor(crsList);

        /*penalty_count pc = new penalty_count();
        pc.penaltyCount(stuList, totalStu, 1);*/
