...
public class NetworkLayerServer {
    ...
    public static synchronized ArrayList<Integer> routerOrder(int startingRouterId)
    {
        ArrayList<Integer> routeTree = new ArrayList<Integer>();

        routeTree.add(startingRouterId);
        for(int i=0; i<routers.size(); i++)
        {
            if(routers.get(i).getRouterId() != startingRouterId)
            {
                routeTree.add(routers.get(i).getRouterId());
            }
        }
    ...
    public static synchronized void DVR(int startingRouterId) {
        ...
        */
        ...
        ArrayList<Integer> routerTree = routerOrder(startingRouterId);
        ...
        while(true)
        {
            ...
            for(int r=0; r<routerTree.size(); r++)
            {
                //int this_rid = routers.get(r).getRouterId();
                int this_rid = routerTree.get(r);
                Router thisRouter = routerMap.get(this_rid);
                ArrayList<RoutingTableEntry> routingTable = routerMap.get(this_rid).getRoutingTable();

                ArrayList<Integer> neighborRouterIDs = routerMap.get(this_rid).getNeighborRouterIDs();

                for(int i=0; i<neighborRouterIDs.size(); i++)
                {
                    int neigh_rId = neighborRouterIDs.get(i);
                    Router neighRouter = routerMap.get(neigh_rId);

                    //if(neighRouter.getState() && thisRouter.getState())
                    if(neighRouter.getState())
                    {
                        if(neighRouter.sfupdateRoutingTable(thisRouter))
                        {
                            convergence = true;
                        }
                    }
            ...
            if(convergence == false || count == Constants.INFINITY)
            {
                break;
            }
        }
        ...
        {
            System.out.println("routerId: " + routers.get(r).getRouterId());
            ArrayList<RoutingTableEntry> routingTable = routers.get(r).getRoutingTable();;
    ...
    public static synchronized void simpleDVR(int startingRouterId) {
        ...
        while(true)
        {
            ...
            for(int r=0; r<routerTree.size(); r++)
            {
                int this_rid = routerTree.get(r);
                Router thisRouter = routerMap.get(this_rid);
                //System.out.println("this_rid: " + thisRouter.getRouterId());
                //ArrayList<RoutingTableEntry> routingTable = routerMap.get(this_rid).getRoutingTable();
                ...
                for(int i=0; i<neighborRouterIDs.size(); i++)
                {
                    int neigh_rId = neighborRouterIDs.get(i);
                    Router neighRouter = routerMap.get(neigh_rId);
                    //System.out.println(neighRouter.getRouterId());
                    //if(neighRouter.getState() && thisRouter.getState())
                    if(neighRouter.getState())
                    {
                        if(neighRouter.updateRoutingTable(thisRouter))
                        {
                            convergence = true;
                        }
                    }
            ...
            if(convergence == false || countToInf == Constants.INFINITY)
            {
                break;
            }
    ...
    public static EndDevice getClientDeviceSetup() {
        ...
        System.out.println("(form_Server)Device : " + ip + "::::" + gateway);
        return device;
    }
    ...
    public static void readTopology() {
        ...
        try {
            ...
            //inputFile = new Scanner(new File("tempTop.txt"));
            //skip first 27 lines
            int skipLines = 27;
